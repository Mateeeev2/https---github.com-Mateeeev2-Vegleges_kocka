...
function drop(event) {
  ...
  for (let x = 0; x < shapeWidth; x++) {
    for (let y = 0; y < shapeHeight; y++) {
      ...
      if (blockX < 0 || blockX >= tableWidth || blockY < 0 || blockY >= tableWidth) {
        ...
        errormessage.innerHTML = "Az alakzat nem eshet a pályán kívül"
        setTimeout(() => {
  ...
  if (!canPlace) {
    errormessage.innerHTML = "Az alakzatot itt nem helyezheted el!"
    setTimeout(() => {
      errormessage.innerHTML = " "
    }, 1000)
    return;
...
function Skip(){
  if(currentMap < 3){
    currentMap++;
        ...
        stageLabel.innerHTML = currentMap + ". Szint"
...
function congratulationsMessage() {
  ...
  if (currentMap == 3) {
    congratsMessage.textContent = 'Szép munka katona a küldetés teljesítve! Haza mehet!';
    shapesContainer.appendChild(congratsMessage);
  } else{
    congratsMessage.textContent = `GG SÜÜÜÜ NYOMOD NEM ADOD FEL! ${currentMap}.`;
...
function blockShape() {
  ...
  blockShapeData.forEach(coord => {
    ...
    if (index < cells.length) {
      cells[index].style.backgroundColor = 'white'; // A kocka színének feketére állítása
    }
  });
}
